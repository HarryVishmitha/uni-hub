...
class StoreAppointmentRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        return $section && $this->user()->can('create', [Appointment::class, $section]);
    }

    protected function prepareForValidation(): void
    {
        if (! $this->filled('assigned_at')) {
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            if ($section && $assignee) {
                ...
                if ($section->appointments()->where('user_id', $assigneeId)->where('role', $role)->exists()) {
                    $validator->errors()->add('user_id', 'This user already has the specified role on the section.');
                }

                $currentLoad = $section->appointments()->sum('load_percent');
